...
with open(filename, encoding="cp949") as file:
    ...
    for row in reader:
        try:
            ...
            if rain > 0:
                current_event_rain += rain
            else:
                if current_event_rain > max_event_rain:
                    max_event_rain = current_event_rain
                current_event_rain = 0
...
print(f"최장 연속 강우일수: {max_streak}일")
print(f"강우 이벤트 중 최대 강수량: {max_event_rain:.1f}mm")